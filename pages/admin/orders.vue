  ...
  <div class="flex h-screen">
    ...
    <LayoutAdmin class="admin-layout">
      <div class="main-content flex-1 overflow-y-auto p-6">
        ...
        <!-- Order Summary Cards -->
        <div v-if="!isLoading && !errorMessage" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div v-if="!isLoading && !errorMessage" class="mb-8">
          ...
          <div class="flex justify-between items-center mb-4 relative">
            ...
            <div class="relative w-[350px]">
              <input v-model="searchQuery" type="text" placeholder="Search: Order ID / Date / Customer Name"
          ...
          <div class="overflow-x-auto bg-white border border-gray-300 rounded-md shadow-sm">
              ...
              <tbody>
                <tr v-for="order in paginatedOrders" :key="order.id"
                  ...
                  <td class="py-4 px-4 text-center">
                    <select v-model="order.orderStatus" @click.stop
                      @change="updateOrderStatusByUser(order.id, order.orderStatus)"
                      class="px-3 py-1 rounded-full text-white" :class="{
            ...
            <div class="flex items-center justify-between px-4 py-3 bg-gray-50 border-t">
              ...
              <div class="flex gap-2">
                <button @click="currentPage--" :disabled="currentPage === 1" class="px-3 py-1 rounded border"
                ...
                <button @click="currentPage++" :disabled="currentPage === totalPages" class="px-3 py-1 rounded border"
    ...
    <div v-if="isModalVisible" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50"
      @click="closeModal">
      <div class="bg-white w-1/2 rounded-lg shadow-lg p-6 transform transition-all duration-300"
        :class="isModalVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'" @click.stop>
        ...
        <div>
          <template v-if="selectedOrder.orderItem.length > 0">
            <ul>
              <li v-for="product in selectedOrder.orderItem" :key="product.id"
                ...
                <div class="flex items-center space-x-4">
                  <img :src="product.product.url" alt="Product Image" class="w-10 h-10 rounded-full object-cover" />
                  <div>
                ...
                <button @click.stop="confirmUnavailableProduct(product.id)"
          ...
          <template v-else>
            <div class="text-center py-4">
              <p class="text-lg font-medium text-gray-700">An admin has set all the products in this order as
                unavailable.</p>
            </div>
          </template>
        </div>

        <button @click="closeModal" class="mt-6 bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
          Close
        </button>
      </div>
    </div>
    ...
    <div v-if="isProductModalVisible"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" @click="closeProductModal">
      <div class="bg-white w-1/3 rounded-lg shadow-lg p-6 transform transition-all duration-300"
        :class="isProductModalVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'" @click.stop>
        ...
        <div v-if="selectedProduct" class="space-y-4">
          <img :src="selectedProduct.url" :alt="selectedProduct.title" class="w-full h-48 object-cover rounded-lg" />
    ...
    <div v-if="isConfirmModalVisible"
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]" @click="closeConfirmModal">
      <div class="bg-white w-1/3 rounded-lg shadow-lg p-6 transform transition-all duration-300"
        :class="isConfirmModalVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'" @click.stop>
        <h2 class="text-2xl font-bold mb-4">Confirm Action</h2>
        <p>Are you sure you want to mark this product as unavailable? This will remove the item from the buyer's
          order.</p>
        <div class="flex justify-end space-x-4 mt-6">
          <button @click="closeConfirmModal" class="bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700">
            Cancel
          </button>
          <button @click="deleteProduct(selectedProductId)"
